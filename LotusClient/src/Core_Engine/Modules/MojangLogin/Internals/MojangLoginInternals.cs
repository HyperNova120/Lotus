...
namespace LotusCore.Modules.MojangLogin.Internals
{
    public class MojangLoginInternals
    {
        ...
        {
            //TODO
            //setup broswer based fallback for OAuth
            ...
            while (true)
            {
                try
                {
                    int port = (rand.Next() % 60000) + 1024; // get rand port outside of lower port range
                    listener.Prefixes.Add($"{redirectUri}:{port}");
                ...
                {
                    Logging.LogError($"HTTP Listner Failed: {e}");
                    ++counter;
                    if (counter == 3)
                    {
                        Logging.LogError("Creation of HttpListener Failed 3 times");
                        return null;
                    }
                }
            }
